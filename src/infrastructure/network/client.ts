import {ErrorResponse} from '@/shared/types/remote/error-response'
...
client.interceptors.response.use(
  (response) => {
    ...
    const elapsedMs = request.metadata ? Date.now() - request.metadata.startTime : undefined
    console.log(`[API][Response] ${getRequestLabel(response.config)} ${response.status}`, {
      elapsedMs,
      data: response.data,
    })
    return response
  ...
  (error: any) => {
    ...
    const extractedMessage: string | undefined = payload?.message ?? respData?.message ?? error.message

    const rawResponseCode = payload?.responseCode ?? respData?.responseCode ?? null
    ...
    const translationLabel = payload?.translationLabel ?? respData?.translationLabel ?? 'msg_unknown_error'

    // Try to translate the translation label using i18n
    let translatedLabelText: string | undefined
    try {
      translatedLabelText = i18next && typeof i18next.t === 'function' ? i18next.t(translationLabel) : undefined
    } catch (e) {
      translatedLabelText = undefined
    }

    const finalMessage = (translatedLabelText && translatedLabelText !== translationLabel ? translatedLabelText : undefined) ?? extractedMessage ?? 'An unknown error occurred'
    ...
    console.log(`[API][Response Error] ${request ? getRequestLabel(request) : 'UNKNOWN'}`, errorResponse, {translatedLabelText}, payload, {
      status: error.response?.status,
