...
function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  async function initAppTheme() {
    const cacheColorScheme = await KeystoreClient.getValueFor(
      KeystoreKeys.THEME_COLOR_SCHEME,
    );
    const cacheColorTheme = await KeystoreClient.getValueFor(
      KeystoreKeys.THEME_COLOR_THEME,
    );
    const colorScheme = cacheColorScheme
      ? (cacheColorScheme as ColorScheme)
      : isDarkMode
        ? "dark"
        : "light";
    const colorTheme = cacheColorTheme
      ? (cacheColorTheme as ColorTheme)
      : ("red" as ColorTheme);
  ...
  const setColorTheme = useCallback(async (theme: ColorTheme) => {
    ...
    const colorScheme = await KeystoreClient.getValueFor(
      KeystoreKeys.THEME_COLOR_SCHEME,
    );
    setTheme((prevTheme) => ({
      ...prevTheme,
  ...
  const setColorScheme = useCallback(async (scheme: ColorScheme) => {
    ...
    const colorTheme = await KeystoreClient.getValueFor(
      KeystoreKeys.THEME_COLOR_THEME,
    );
    setTheme((prevTheme) => ({
      ...prevTheme,
