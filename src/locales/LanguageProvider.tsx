...
export const LanguageProvider: React.FC<{children: React.ReactNode}> = ({children}) => {
  ...
  useEffect(() => {
    const loadLanguage = async () => {
      try {
        const storedLanguage = await KeystoreClient.getValueFor(KeystoreKeys.APP_LANGUAGE)
        if (storedLanguage) {
          await i18n.changeLanguage(storedLanguage)
          setLanguageState(storedLanguage)
        } else {
          // If no language is stored, save the default one
          await KeystoreClient.save(KeystoreKeys.APP_LANGUAGE, i18n.language || 'en')
        }
      } catch (error) {
  ...
  return <LanguageContext.Provider value={{language, setLanguage, isLanguageLoaded}}>{children}</LanguageContext.Provider>
}
